(function () {
  ...
  masterPanel.style.cssText =
    ...
    'min-width:220px;box-shadow:0 6px 24px rgba(0,0,0,.7);user-select:none;';
  ...
  masterPanel.innerHTML =
    ...
    '</label>' +
    '<label style="display:flex;align-items:center;gap:6px;cursor:pointer;">' +
    '<input type="checkbox" id="pm-cb-trile" style="width:14px;height:14px;cursor:pointer;">' +
    '<span>🔮 Trackeo Alcanzapelotas</span>' +
    '</label>' +
    '<label style="display:flex;align-items:center;gap:6px;cursor:pointer;">' +
  ...
  var cbTrile = masterPanel.querySelector('#pm-cb-trile');
  ...
  function applyMin() {
    ...
    masterPanel.style.minWidth = minimized ? '0' : '220px';
  ...
  // ============================================================
  // MÓDULO 8: TRILE PREDICTOR (Trackeo Alcanzapelotas)
  // ============================================================
  //
  // Calcula la respuesta simulando los swaps. El juego expone
  // "pos0" (inicio) y "swaps" (lista de intercambios) desde el
  // principio. Simula el recorrido y dibuja un recuadro celeste
  // alrededor de la casilla correcta; al resolverse la ronda,
  // pasa a verde (acierto) o rojo (no).
  // ============================================================

  var modulesTrilePredictor = (function () {
    var started = false;
    var timer = null;
    ...
    var lastPos0 = undefined;
    var predicted = null;
    var verified = false;

    function getFiberRoot() {
      var buttons = Array.prototype.slice.call(document.querySelectorAll('button'))
        .filter(function (b) { return /\d+º/.test(b.textContent); });
    ...
    function deepFindTrile(val, depth, seen) {
      if (depth < 0 || val === null || typeof val !== 'object') return null;
      if (seen.has(val)) return null;
      seen.add(val);
      if ('finalPos' in val && 'pos0' in val) return val;
      for (var k in val) {
        if (!Object.prototype.hasOwnProperty.call(val, k)) continue;
        var v = val[k];
        if (v === null || typeof v !== 'object') continue;
        if (typeof Node !== 'undefined' && v instanceof Node) continue;
        var found = deepFindTrile(v, depth - 1, seen);
        if (found) return found;
      }
      return null;
    }

    function findTrileState() {
      var fiber = getFiberRoot();
      if (!fiber) return null;
      var level = 0, current = fiber;
      while (current && level < 14) {
        var hook = current.memoizedState;
        var hi = 0;
        while (hook && hi < 40) {
          var found = deepFindTrile(hook.memoizedState, 6, new Set());
          if (found) return found;
          hook = hook.next;
          hi++;
        }
    ...
    function positionHighlight() {
      if (!predictedButton || !highlight) { if (highlight) highlight.style.display = 'none'; return; }
      var r = predictedButton.getBoundingClientRect();
    ...
    function refresh() {
      var trile = findTrileState();
      if (!trile || typeof trile.pos0 !== 'number' || !Array.isArray(trile.swaps)) {
        return;
      }

      if (trile.pos0 !== lastPos0) {
        lastPos0 = trile.pos0;
        predicted = simulate(trile.pos0, trile.swaps);
        verified = false;

        var buttons = findSlotButtons();
        predictedButton = buttons[predicted] || null;
        highlight.style.borderColor = '#2a9d8f';
        highlight.style.boxShadow = '0 0 0 3px rgba(42,157,143,.35), 0 0 20px rgba(42,157,143,.8)';
        positionHighlight();

        statusTag.textContent = '🔮 calculado, esperando confirmación...';
        console.log('%c🔮 Predicción: posición ' + (predicted + 1) + ' (arrancó en ' + (trile.pos0 + 1) + ')', 'background:#2a9d8f;color:#000;padding:2px 6px;');
      }

      if (!verified && typeof trile.finalPos === 'number') {
        verified = true;
        var coincide = (trile.finalPos === predicted);
        statusTag.textContent = coincide ? '✅ coincidió' : '❌ no coincidió (real: ' + (trile.finalPos + 1) + ')';
        highlight.style.borderColor = coincide ? '#2a9d8f' : '#e63946';
        highlight.style.boxShadow = coincide
          ? '0 0 0 3px rgba(42,157,143,.35), 0 0 20px rgba(42,157,143,.8)'
          : '0 0 0 3px rgba(230,57,70,.35), 0 0 20px rgba(230,57,70,.8)';
        console.log(coincide
          ? '%c✅ Predicción correcta'
          : '%c❌ Predicción incorrecta. Real: ' + (trile.finalPos + 1) + ', predicho: ' + (predicted + 1),
          'background:' + (coincide ? '#2a9d8f' : '#e63946') + ';color:#000;padding:2px 6px;');
      }
    }
    ...
    function start() {
      ...
      highlight.style.cssText =
        ...
        'border:4px solid #2a9d8f;border-radius:14px;' +
        'box-shadow:0 0 0 3px rgba(42,157,143,.35), 0 0 20px rgba(42,157,143,.8);' +
      ...
      statusTag.style.cssText =
        ...
        'background:#0a0a0a;color:#888;font:11px monospace;' +
        'border:1px solid #555;border-radius:6px;padding:4px 8px;';
      statusTag.textContent = '🔮 esperando ronda...';
      document.body.appendChild(statusTag);

      onScroll = function () { positionHighlight(); };
      onResize = function () { positionHighlight(); };
      window.addEventListener('scroll', onScroll, true);
      window.addEventListener('resize', onResize);
      layoutTimer = setInterval(positionHighlight, 200);
      ...
      console.log('%c Trile Predictor activo. ', 'background:#0a0a0a;color:#2a9d8f;padding:2px 6px;border-radius:4px;');
    }
    ...
    function stop() {
      ...
      if (statusTag) { statusTag.remove(); statusTag = null; }
      predictedButton = null;
      console.log('Trile Predictor detenido.');
    }
  ...
  // ============================================================
  // MÓDULO 9: MEMO TRACKER (juego de cartas por pares)
  // ============================================================
  ...
  bindCheckbox(cbTrile, modulesTrilePredictor);
  bindCheckbox(cbMemoTracker, modulesMemoTracker);
  ...
  window.PanelManager = {
    ...
    trilePredictor: modulesTrilePredictor,
